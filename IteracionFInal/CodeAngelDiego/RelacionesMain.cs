...
namespace WindowsFormsApplication2
{
    public partial class RelacionesMain : Form
    {
        ...
        {
            if (ComprobarLimiteRelacion())
            {
                MessageBox.Show("Max number of attributes reached, please delete one to create another one");
                return; // No se permite abrir el formulario
            }
        ...
        private bool ComprobarLimiteRelacion()
        {
            using (var dbContext = new DataClasses1DataContext()) 
            {
                // Contar los atributos en la base de datos
                int totalRelacion = dbContext.Relacion.Count();

                // Verificar si ya existen 3 o más atributos
                return totalRelacion >= 3;
            }
        }
